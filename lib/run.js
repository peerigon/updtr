    ...
    exec("npm outdated --json --long --depth=0", function (err, stdout, stderr) {
        ...
        infos = Object.keys(outdated)
            ...
            .filter(function (info) {
                var isGitDependency = info.latest === "git";
                var alreadyInstalled = info.updateTo === info.current;
                var isUnstable = false;
                var excludedModules;
                var isExcluded = false;
                var isCurrentGraterThanUpdateTo = false;
                ...
                if (info.current && info.updateTo !== "git" && semver.gt(info.current, info.updateTo)) {
                    isCurrentGraterThanUpdateTo = true;
                }

                return !(isGitDependency || alreadyInstalled || isUnstable || isExcluded || isCurrentGraterThanUpdateTo);
            });
