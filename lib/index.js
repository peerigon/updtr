...
exec("npm outdated --json --long --depth=0", { encoding: "utf8" }, function (err, stdout, stderr) {
    ...
    function createTask(info, index, outdatedModules) {
        ...
        return function (done) {
            var msg = colors.grey("(" + index + "/" + outdatedModules.length + ")") + "\t" + info.name + "\tupdating ".grey + info.current + " " + symbols.arrow.gray + " " + info.latest + " ... ".grey;
            ...
            async.series({
                ...
                if (err) {
                    log(
                        msg = colors.grey("(" + index + "/" + outdatedModules.length + ")") + "\t" + info.name.red + "\tupdating ".grey + info.current + " " + symbols.arrow.gray + " " + info.latest + " ... ".grey +
